...
class DPOMixTrainer(Trainer):
    ...
    def concatenated_inputs(
        ...
        # ]
        if vision:
            # batch["images"].extend(batch["mix_images"])
    ...
    def dpo_loss(
        ...
        label_smoothing: Union[float, list, torch.Tensor] = 1.0
        ...
        if self.loss_type == "sigmoid":
            if isinstance(label_smoothing, list):
                label_smoothing = torch.tensor(
                    label_smoothing,
                    device=logits.device,
                    dtype=logits.dtype
                )
            elif isinstance(label_smoothing, (float, int)):
                label_smoothing = torch.full_like(logits, fill_value=label_smoothing, dtype=logits.dtype, device=logits.device)
            elif isinstance(label_smoothing, torch.Tensor):
    ...
    def get_batch_loss_metrics(
        ...
        sft_loss = unscaled_sft_loss * self.gamma if not vision else unscaled_sft_loss * 0
    ...
    def compute_loss(
        ...
        with compute_loss_context_manager():
            ...
            loss_vision, metrics_vision = self.get_batch_loss_metrics(model, inputs, train_eval="train", vision=True)
            metrics[f"losses/vision_dpo"] = loss_vision.cpu()
            loss += 0.5 * loss_vision
