...
class RewardTrainer(Trainer):
    ...
    def compute_loss(
        ...
        rewards_chosen, loss_critic_positive = model(
        ...
        rewards_rejected, loss_critic_negative = model(
        ...
        # Check if `is_tie` exists in the inputs
        if "is_tie" in inputs:
            is_tie_mask = torch.tensor(inputs["is_tie"], device=rewards_chosen.device, dtype=torch.bool)
            non_tie_mask = ~is_tie_mask  # Non-tie mask for standard reward processing
        else:
            # If no `is_tie` is provided, treat all samples as non-tie
            is_tie_mask = None
            non_tie_mask = torch.ones_like(rewards_chosen, dtype=torch.bool, device=rewards_chosen.device)

        # Compute reward loss for non-tie samples
        if non_tie_mask.any():
            rewards_chosen_non_tie = rewards_chosen[non_tie_mask]
            rewards_rejected_non_tie = rewards_rejected[non_tie_mask]
            if "margin" in inputs:
                # Use margin if provided
                loss_reward = -nn.functional.logsigmoid(
                    rewards_chosen_non_tie - rewards_rejected_non_tie - inputs["margin"]
                ).mean()
            else:
                # Standard reward loss without margin
                loss_reward = -nn.functional.logsigmoid(
                    rewards_chosen_non_tie - rewards_rejected_non_tie
                ).mean()
        else:
            # No non-tie samples, set reward loss to zero
            loss_reward = torch.tensor(0.0, device=rewards_chosen.device)

        # Compute tie loss if `is_tie` exists and there are tie samples
        if is_tie_mask is not None and is_tie_mask.any():
            rewards_chosen_tie = rewards_chosen[is_tie_mask]
            rewards_rejected_tie = rewards_rejected[is_tie_mask]
            # Tie loss: mean squared difference to make rewards as close as possible
            tie_rewards_diff = (rewards_chosen_tie - rewards_rejected_tie) ** 2
            tie_loss = tie_rewards_diff.mean()
            # Add tie loss weighted by tie_weight
            loss_reward += self.args.tie_weight * tie_loss
        else:
            # No tie samples, set tie loss to zero
            tie_loss = torch.tensor(0.0, device=rewards_chosen.device)

        # Apply optional floating rewards weight
        if self.args.float_rewards_weight is not None:
            loss_reward *= self.args.float_rewards_weight

        # Apply optional center rewards coefficient
        if self.args.center_rewards_coefficient is not None:
            loss_reward += self.args.center_rewards_coefficient * torch.mean((rewards_chosen + rewards_rejected) ** 2)

        # Compute critic loss if specified
        if self.args.critic_rewards_weight is not None:
            ls_factor = torch.tensor(
                inputs['no_critic'],
                device=loss_reward.device,
                dtype=loss_reward.dtype
            )

            loss_critic = self.args.critic_rewards_weight * (loss_critic_positive + loss_critic_negative) * (1 - ls_factor)
        else:
            loss_critic = 0

        # Combine reward loss and critic loss
        loss = loss_reward + loss_critic.mean()

        metrics = {}
        prefix = 'train'
        
        
        # Compute reward accuracies for non-tie samples only
        if non_tie_mask.any():
            reward_accuracies = (rewards_chosen[non_tie_mask] > rewards_rejected[non_tie_mask]).float()
        else:
            reward_accuracies = torch.tensor(0.0, device=rewards_chosen.device)
        
        # Compute tie margin for tie samples (if any)
        if is_tie_mask is not None and is_tie_mask.any():
            tie_margins = torch.abs(rewards_chosen[is_tie_mask] - rewards_rejected[is_tie_mask])
            tie_margin_mean = tie_margins.mean()  # Mean tie margin
        else:
            tie_margin_mean = torch.tensor(0.0, device=rewards_chosen.device)
        ...
        loss_critic_positive = all_gather_tensor(loss_critic_positive)
        loss_critic_negative = all_gather_tensor(loss_critic_negative)
        loss_reward = all_gather_tensor(loss_reward)
        rewards_chosen = all_gather_tensor(rewards_chosen)
        rewards_rejected = all_gather_tensor(rewards_rejected)
        reward_accuracies = all_gather_tensor(reward_accuracies)
        tie_margin_mean = all_gather_tensor(tie_margin_mean)
        metrics[f"{prefix}loss/critic_chosen"] = loss_critic_positive.mean().cpu()
        metrics[f"{prefix}loss/critic_rejected"] = loss_critic_negative.mean().cpu()
        metrics[f"{prefix}loss/reward"] = loss_critic_positive.mean().cpu()
        ...
        if is_tie_mask is not None and is_tie_mask.any():
            metrics[f"{prefix}loss/tie"] = tie_loss.mean().cpu()  # Log tie loss
            metrics[f"{prefix}rewards/tie_margin"] = tie_margin_mean.mean().cpu()  # Log tie margin
