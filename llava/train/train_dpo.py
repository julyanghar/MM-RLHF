...
from llava.train.llava_trainer import LLaVADPOTrainer, LLaVARMTrainer
from llava import conversation as conversation_lib
from llava.model import *
from llava.model.language_model.llava_qwen import LlavaQwenConfig, RewardHead
...
class ModelArguments:
    ...
    # deciding wdpo_forwardch part of the multimodal model to tune, will overwrite other previous settings
    ...
    is_rm: Optional[bool] = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    center_rewards_coefficient: float = field(default=0.01, metadata={"help": "Coefficient for center rewards"})
    critic_rewards_weight: float = field(default=1.0, metadata={"help": "Weight for critic loss"})
    
    vision_dpo_weight: float = field(default=0.0, metadata={"help": "Weight for vision DPO loss"})
    vision_dpo_pos_pos: bool = field(default=False, metadata={"help": "If True, apply vision DPO for positive-positive pairs"})
    
    float_rewards_weight: float = field(default=1.0, metadata={"help": "Weight for reward loss"})
    generate_during_eval: bool = field(default=False, metadata={"help": "If True, generate during evaluation"})
    precompute_ref_log_probs: bool = field(default=False, metadata={"help": "If True, precompute ref logits for later DPO training"})
...
def make_conv_rm(prompt, answer, reason, has_image=True):
    critic_prompt = (
...
class DPODataset(Dataset):
    ...
    def lengths(self):
        ...
        for sample in self.list_data_dict:
            # Calculate the length of the prompt, chosen, and rejected text
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if "mixed_up_image" in sources[0]:
                image_file = self.list_data_dict[i]["mixed_up_image"]
                if type(image_file) is list:
                    mix_image = [self.process_image(f) for f in image_file]
                else:
                    mix_image = [self.process_image(image_file)]
        ...
        # Add reference logits if available
        if "reference_chosen_logp" in data_dict and "reference_rejected_logp" in data_dict:
            data_dict["reference_chosen_logp"] = torch.tensor(data_dict["reference_chosen_logp"], dtype=torch.float)
            data_dict["reference_rejected_logp"] = torch.tensor(data_dict["reference_rejected_logp"], dtype=torch.float)
        if "mix_reference_chosen_logp" in data_dict and "mix_reference_rejected_logp" in data_dict:
            data_dict["mix_reference_chosen_logp"] = torch.tensor(data_dict["mix_reference_chosen_logp"], dtype=torch.float)
            data_dict["mix_reference_rejected_logp"] = torch.tensor(data_dict["mix_reference_rejected_logp"], dtype=torch.float)
        if "chosen_reason" in data_dict and "rejected_reason" in data_dict:
            data_dict["chosen_reason"] = data_dict["chosen_reason"]
            data_dict["rejected_reason"] = data_dict["rejected_reason"]
        if "is_tie" in data_dict:
            data_dict["is_tie"] = data_dict["is_tie"]

        # prompt exist in the data
        data_dict["has_image"] = has_image
        data_dict["ls_factor"] = self.list_data_dict[i]['ls_factor'] if "ls_factor" in self.list_data_dict[i] else 1.0
        return data_dict
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def collate(self, batch):
        ...
        for k in batch[0].keys():
            ...
            elif k in ["reference_chosen_logp", "reference_rejected_logp", 'mix_reference_chosen_logp', 'mix_reference_rejected_logp']:
                # Stack logits directly without padding
                padded_batch[k] = torch.stack([ex[k] for ex in batch])
            else:
    ...
    def tokenize_batch_element(self, prompt: str, chosen: str, rejected: str, chosen_reason: str = None, rejected_reason: str = None, has_image: bool = True) -> Dict:
        ...
        if not self.is_rm or chosen_reason is None:
            chosen_sources = make_conv(prompt, chosen)
            rejected_sources = make_conv(prompt, rejected)
        else:
            chosen_sources = make_conv_rm(prompt, chosen, chosen_reason, has_image)
            rejected_sources = make_conv_rm(prompt, rejected, rejected_reason, has_image)
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            or "yi" in model_args.model_name_or_path.lower()
            or "nous-hermes" in model_args.model_name_or_path.lower()
            or "llava-v1.5" in model_args.model_name_or_path.lower()
            ...
            if not training_args.precompute_ref_log_probs and "zero3" in training_args.deepspeed and not model_args.is_rm:
    ...
    if model_args.is_rm:
        model.rm_head = RewardHead(model.config, 1)
        model.config.is_rm = True
        return model, None
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.is_rm = model_args.is_rm
    ...
    if model_args.is_rm:
        ...
        trainer = LLaVADPOTrainer(
            ...
            ref_data=training_args.ref_data
        )
